...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Введите цвет для статуса {} (в формате HEX, например #FF0000):".format(
            STATUSES[0]
        )
    )
...
async def status_color(message: Message, state: FSMContext):
    ...
    if index < len(STATUSES):
        ...
        await message.answer(
            f"Введите цвет для статуса {STATUSES[index]} (HEX):"
        )
        return
    await state.update_data(statuses=statuses)
    await message.answer("Введите цвет текста (HEX):")
...
async def text_size(message: Message, state: FSMContext):
    ...
    await message.answer("Введите цвет фона панели (HEX):")
...
async def panel_bg(message: Message, state: FSMContext):
    ...
    await message.answer("Введите количество колонок сетки:")
...
async def columns(message: Message, state: FSMContext):
    ...
    await message.answer("Введите количество строк сетки:")
...
async def rows(message: Message, state: FSMContext):
    ...
    await message.answer("Включить мигание при изменении? (y/n):")
...
async def blink(message: Message, state: FSMContext):
    ...
    await message.answer("Группировать заказы? (y/n):")
...
async def group(message: Message, state: FSMContext):
    ...
    await message.answer("Показывать время? (y/n):")
